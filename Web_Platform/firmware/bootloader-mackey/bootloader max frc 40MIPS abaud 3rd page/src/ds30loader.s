...
;------------------------------------------------------------------------------
		...
		.equ	STARTADDR,	( FLASHSIZE - 3 * PAGESIZE * 2 ) 			/*place bootloader in last program page*/
...
;------------------------------------------------------------------------------
		...
		;------------------------------------------------------------------------------
		; Tune internal oscillator to approx. 8.2267625MHz
		; This minimizes write time that depends on frc
		; Datasheet pages 149, 329, 74
		;------------------------------------------------------------------------------
		mov		#31, W0
		mov		W0, OSCTUN	
		...
		;------------------------------------------------------------------------------
		; Set PLL so we get close to 40MIPS
		;------------------------------------------------------------------------------
		;PLLPOST=2
...
waitforPLLlock: btss 	OSCCON, #LOCK		
		...
		.ifdef HAS_PPS
			...
			;LD2 on during bootload time
			bclr 	TRISA, #TRISA8
			bclr 	TRISA, #TRISA9
			bclr 	LATA, #LATA8		;led 1 off
			bset 	LATA, #LATA9
		.endif	
...
anotrcv:dec 	WDEL2, WDEL2
		...
		bra		exit		
		; Enable transmit and send OK
abaudok:bset 	USTA, #UTXEN		;enable TX after RX, to avoid backpowering FTDI chip			
...
exit:	bclr	UIFS, #URXIF		;clear uart received interupt flag
		...
		bclr 	LATA, #LATA9		;turn off LD1, done with bootloader
