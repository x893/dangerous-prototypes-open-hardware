...
namespace ds30Loader 
{
    ...
    // Class: clsHex24FJ
    //-------------------------------------------------------------------------
    public class clsHex24FJ : clsHex 
    {
        //---------------------------------------------------------------------
        // Size suffixes
        //---------------------------------------------------------------------
        ...
        // Constructor()
        //---------------------------------------------------------------------
        public clsHex24FJ( string pstrFilename ) : base( pstrFilename ) 
        {
            // Size buffer arrays
        ...
        {
            return Convert.ToInt32( pbWriteProgram ) * ( iProgPagesUsed * pobjDevice.pageSizeR * pobjDevice.rowsizeW * 3 );
        }// BytesToWrite()
        ...
        override public void ParseHexFile( clsParseSettings pobjSettings, int iTabLevel, ref bool pbResult )
        {
            ...
            //--------------------------------------------------------------------------
            // Last page used?
            // Configs resides in last words in last page
            //--------------------------------------------------------------------------
            int iLastPage = pobjSettings.device.flashSizeP / ( 2 * pobjSettings.device.pageSizeR * pobjSettings.device.rowsizeW ) - 1;
            if ( bProgPageUsed[iLastPage] == true ) {
            }
