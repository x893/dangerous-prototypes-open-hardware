...
namespace ds30Loader
{
	...
	public class clsHex30F : clsHex
	{
		...
		// 
		//---------------------------------------------------------------------
		// 1 row = 32 words = 64 pcu = 96 bytes 


		//---------------------------------------------------------------------
		// Constants
		//---------------------------------------------------------------------
		public const int iProgRowsUsedBufferSize = 1536;
        public const int iProgMemBufferSize = iProgRowsUsedBufferSize * 32 * 3;
		...
		// Constructor()
		//---------------------------------------------------------------------
		public clsHex30F( string pstrFilename ) : base ( pstrFilename )
		{
			// Size buffer arrays
			bProgRowUsed = new bool[ iProgRowsUsedBufferSize ];
		...
		override public void ParseHexFile( clsParseSettings pobjSettings, int iTabLevel, ref bool pbResult )
		{
			...
			//--------------------------------------------------------------------------
			for ( iIter = 0; iIter < iProgRowsUsedBufferSize; iIter++ ) bProgRowUsed[iIter] = false;
			...
			for ( iIter = 0; iIter < iConfigMemBufferSize; iIter++ ) iConfigMem[iIter] = 0xFF;			
			...
			// Calculate adresses
			//--------------------------------------------------------------------------
			int iBootloaderStartRow = (pobjSettings.device.flashSizeP/2 - pobjSettings.device.rowsizeW * pobjSettings.bootloaderSizeR) / pobjSettings.device.rowsizeW;
            int iBootloaderEndRow = iBootloaderStartRow + pobjSettings.bootloaderSizeR - 1;
			int iBootloaderStartAdress = pobjSettings.device.flashSizeP - pobjSettings.device.rowsizeW * pobjSettings.bootloaderSizeR * 2;
			...
			//--------------------------------------------------------------------------
			iProgRowsUsed = 0;
			for ( iIter = 0; iIter < iProgRowsUsedBufferSize; iIter++ ) {
				if ( bProgRowUsed[iIter] == true ) {
					++iProgRowsUsed;
			...
			//--------------------------------------------------------------------------
			// Set GOTO to bootloader at 0x0000, upper-low-high order
			//--------------------------------------------------------------------------
			if ( pobjSettings.noGoto == false ) {				
				// First goto word
