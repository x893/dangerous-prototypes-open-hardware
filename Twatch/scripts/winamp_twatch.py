...
# via http://ingeration.blogspot.com/2008/05/python-winamp-control-module.html

# twatch code by Andrew Bolin (with some constant definitions copied off the forum).

# matrix orbital code ref: http://www.freewebs.com/x7jay7x/
# twatch ref: http://dangerousprototypes.com/2009/09/17/prototype-network-lcd-backpack-with-lcd-smartie/
...
if __name__ == "__main__":
        ...
        while(1==1):
                ...
                if raw != lastRaw: # if track has changed
                        ...
                        # winamp will only give us combined artist & title
                        # need to handle cases where the seperator is missing
                        # or appears in artist or title string.
                        if raw.find(' - ') != -1:
                                [artist, title] = raw.split(' - ', 1)
                        # remove excess whitespace so we can centre display        
                        artist = artist.strip()
                        title = title.strip()
                        # if too long, text will scroll. add a marker to indicate start/end.
                        if len(artist) > TWATCH_X:
                                artist += " | "
                        if len(title) > TWATCH_X:
                                title += " | "
