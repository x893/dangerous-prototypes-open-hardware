...
namespace Sevenstring.HWInterface
{
    public class BusPirateRawWire
    {
        ...
        public const byte BPC_4Bit=0x30;
        ...
        */
        ...
        /// <returns></returns>
        public bool SendByteBulkTransfer(int length,byte [] dataArray)
        {
        const int DELAY=500;

        myBusPirate.BusPirateSerialPort.Write(new byte[]{(byte)(BPC_BulkTransfer|(length-1))},0,1);

        //if(myBusPirate.SendUartDataAndCheckReply(new byte[]{(byte)(BPC_BulkTransfer|(length-1))},DELAY)==false) return false; //TODO BALIK

        for(int ctr=0;ctr<length;ctr++)
            {
            if(myBusPirate.SendUartDataAndCheckReply(new byte[]{dataArray[ctr]},DELAY)==false) return false;
            }
        return true;
        }


        public bool SendFlexibleBitTransferMSBFirst(int numBits,byte data)
        {
        const int DELAY=500;
        //myBusPirate.BusPirateSerialPort.Write(new byte[]{(byte)(BPC_BulkTransfer|length)},0,1);
        myBusPirate.BusPirateSerialPort.Write(new byte[]{(byte)(BPC_4Bit|(numBits-1))},0,1);
        //if(myBusPirate.SendUartDataAndCheckReply(new byte[]{(byte)(BPC_4Bit|(numBits-1))},DELAY)==false) return false; //TODO Balik
        if(myBusPirate.SendUartDataAndCheckReply( new byte[]{(byte)data},DELAY)==false) return false;
        return true;

#if N_COMP
        if(myBusPirate.SendUartDataAndCheckReply(new byte[]{(byte)(BPC_4Bit|(length-1))},DELAY)==false) return false;

        for(int ctr=0;ctr<length;ctr++)
            {
            if(myBusPirate.SendUartDataAndCheckReply(new byte[]{(byte) (dataArray[ctr]<<0)},DELAY)==false) return false;
            }
        return true;
#endif
        }
        ...
        public byte RolloverDataByte(byte data)
        {
        int RetData=0;
