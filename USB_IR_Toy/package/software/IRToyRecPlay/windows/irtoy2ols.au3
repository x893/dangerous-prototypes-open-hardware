...
;$m=MsgBox(0,"HEXA  ","IRTOYDATA="&$irtoydata &" HEXA  "&$hexa)
$decimalno=dec(StringRight($hexa,4))
;$m=MsgBox(0,"decimal no","decimalno="&$decimalno)

	if $decimalno < 65535 then 
		$size=$size+1
		$absolute= $absolute + $decimalno
;		$m=MsgBox(0,"LOOP ","size="&$size &" absolute=  "&$absolute&" decimalno = "&$decimalno)
	EndIf
WEnd
...
while 1
; irtoydata is 4bytes always geting 0xaaaa 
$irtoydata=FileRead($file1,2)
...
$decimalno=dec(StringRight($hexa,4))
;$m=MsgBox(0," at while 1 final write  ","IRTOYDATA="&$irtoydata &" HEXA  "&$hexa&" decimalno= "&$decimalno)
$irtime=$decimalno
; alternating 0 and 1 note "000000"@
; dtw started from 0 adding alternatively 
if $io = 0 then 
	if $decimalno < 65535 then 
		$dtw = $dtw + $decimalno
